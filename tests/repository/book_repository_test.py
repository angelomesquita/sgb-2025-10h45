...
@pytest.fixture()
def sample_books() -> Iterable[Book]:
    ...
    return [
        Book(isbn="9789789789781", title="Book 1", author=author, publisher=publisher, year=2025, quantity=1),
        Book(isbn="9789789789782", title="Book 2", author=author, publisher=publisher, year=2025, quantity=1),
        Book(isbn="9789789789783", title="Book 3", author=author, publisher=publisher, year=2025, quantity=1),
    ]


@pytest.mark.parametrize(
    "isbn, expected_title, expected_author_name, expected_publisher_legal_name, expected_year, expected_quantity", [
...
def test_get_book_by_id_return_book(sample_books, isbn, expected_title, expected_author_name,
                                    expected_publisher_legal_name, expected_year, expected_quantity):
...
def test_get_book_by_id_raises_error(sample_books):
    """Ensures get_book_by_id() raise BookNotFound when isbn is not found."""
    with patch.object(BookDao, 'get_all', return_value=sample_books) as mock_load_all:
        with pytest.raises(BookNotFoundError, match="Book with isbn X999 not found."):
            BookRepository.get_book_by_isbn(isbn="X999")
        mock_load_all.assert_called_once()
...
def test_search_book_return_empty_iterable_when_book_not_available(sample_books):
    """Ensures search() return the empty list."""
    with patch.object(BookDao, 'get_all', return_value=sample_books) as mock_load_all:
...
def test_decrease_quantity_of_book_raises_error_when_book_not_found(sample_books):
    with patch.object(BookDao, 'get_all', return_value=sample_books):
        with pytest.raises(BookNotFoundError, match="Book with isbn X999 not found."):
            BookRepository.decrease_quantity(isbn="X999")
...
def test_increase_quantity_of_book_raises_error_when_book_not_found(sample_books):
    with patch.object(BookDao, 'get_all', return_value=sample_books):
        with pytest.raises(BookNotFoundError, match="Book with isbn X999 not found."):
            BookRepository.increase_quantity(isbn="X999")
