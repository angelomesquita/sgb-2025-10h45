...
class PublisherModule(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        ttk.Label(form_frame, text="State:").grid(row=1, column=2, padx=0, pady=0, sticky="e")
    ...
    def bind_events(self):
        ...
        self.entry_legal_name.bind("<KeyRelease>", self.validate_entry_legal_name)
        self.entry_city.bind("<KeyRelease>", self.validate_entry_city)
        self.entry_state.bind("<KeyRelease>", self.validate_entry_state)
    ...
    def register_publisher(self):
        ...
        if not PublisherValidator.validate_publisher_id(publisher_id):
            messagebox.showerror("Validation Error", "Invalid Publisher ID. Must be numeric.")
            return

        if not PublisherValidator.validate_legal_name(legal_name):
            messagebox.showerror("Validation Error", "Legal name must be at least 5 characters.")
            return

        if not PublisherValidator.validate_city(city):
            messagebox.showerror("Validation Error", "City must be at least 5 characters.")
            return

        if not PublisherValidator.validate_state(state):
            messagebox.showerror("Validation Error", "State must be at least 2 characters.")
            return

        try:
            self.controller.register(publisher_id=publisher_id, legal_name=legal_name, city=city, state=state)
            messagebox.showinfo("Success", f"Publisher: '{legal_name}' registered successfully.")
            self.load_publishers()
            self.clear_form()
    ...
    def update_publisher(self):
        if not self.selected_publisher_id:
            messagebox.showwarning("Warning", "Select a publisher to update.")
            return
    ...
    def delete_publisher(self):
        if not self.selected_publisher_id:
            messagebox.showwarning("Warning", "Select a publisher to delete.")
            return

        confirm = messagebox.askyesno("Confirm Delete", "Are you sure you to delete this publisher?")
        if not confirm:
            return
    ...
    def load_publishers(self):
        ...
        try:
            ...
            messagebox.showerror("Error", f"Failed to load publishers: {e}")
